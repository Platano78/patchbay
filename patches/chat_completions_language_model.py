...
class ChatCompletionsApiModelHandler(BaseOpenAICompatibleHandler):
    ...
    def _iter_stream_events(self, api_response: Stream[ChatCompletionChunk]) -> Iterator[ProviderEvent]:
        ...
        for chunk in api_response:
            ...
            if text_piece:
                filtered_piece = think_filter.feed(text_piece)
                if filtered_piece:
                    raw_text += filtered_piece
                    yield TextDelta(text=filtered_piece)

        remnant = think_filter.flush()
        if remnant:
            raw_text += remnant
            yield TextDelta(text=remnant)
        if raw_text.strip():
    ...
    def _iter_response_events(self, api_response: Any) -> Iterator[ProviderEvent]:
        ...
        if raw_content:
            think_filter = ThinkTagFilter()
            raw_content = think_filter.feed(raw_content) + think_filter.flush()
        if raw_content:
