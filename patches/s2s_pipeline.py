...
from speech_to_speech.arguments_classes.remote_speech_tts_arguments import (
    RemoteSpeechTTSHandlerArguments,
)
...
class ParsedArguments:
    ...
    remote_speech_tts_handler_kwargs: RemoteSpeechTTSHandlerArguments
...
def parse_arguments() -> ParsedArguments:
    ...
    return ParsedArguments(
        ...
        remote_speech_tts_handler_kwargs=by_type[RemoteSpeechTTSHandlerArguments],
    )
...
def check_mac_settings(module_kwargs: ModuleArguments) -> None:
    if platform == "darwin":
        ...
        if module_kwargs.tts not in ("pocket", "kokoro", "qwen3", "remote-speech"):
            logger.warning(
...
def prepare_all_args(
    ...
    prepare_module_args(
        ...
        remote_speech_tts_handler_kwargs,
    )
    ...
    rename_args(remote_speech_tts_handler_kwargs, "remote_speech")
...
def _build_realtime_pipeline_unit(
    *,
    ...
    remote_speech_tts_kw = deepcopy(remote_speech_tts_handler_kwargs)
    ...
    handlers = _build_pipeline_handlers(
        ...
        remote_speech_tts_handler_kwargs=remote_speech_tts_kw,
        speculative_turns=speculative_turns,
    )
    for h in handlers:
...
def build_pipeline(
    ...
    websocket_streamer_ref: Any = None
    if module_kwargs.mode == "local":
        ...
        websocket_streamer_ref = websocket_streamer
        ...
        pool = [
            _build_realtime_pipeline_unit(
                ...
                remote_speech_tts_handler_kwargs=remote_speech_tts_handler_kwargs,
            )
    ...
    if (
        module_kwargs.mode == "websocket"
        and module_kwargs.llm_backend in ("responses-api", "chat-completions")
        and websocket_streamer_ref is not None
    ):
        ...
        # attribute), so it needs a reference to whichever backend is
        # actually active rather than only recognising Pocket.
        tts_handler = next((h for h in pipeline_handlers if type(h).__module__.startswith("speech_to_speech.TTS.")), None)
...
def get_tts_handler(
    ...
    if module_kwargs.tts == "chatTTS":
    ...
    elif module_kwargs.tts == "remote-speech":
        from speech_to_speech.TTS.remote_speech_tts_handler import RemoteSpeechTTSHandler

        setup_kwargs = dict(vars(remote_speech_tts_handler_kwargs))
        setup_kwargs["pocket_kwargs"] = vars(pocket_tts_handler_kwargs)
        return RemoteSpeechTTSHandler(
            stop_event,
            queue_in=lm_response_queue,
            queue_out=send_audio_chunks_queue,
            setup_args=(should_listen,),
        ...
        raise ValueError("The TTS should be either chatTTS, facebookMMS, pocket, kokoro, qwen3, or remote-speech")
...
def main() -> None:
    ...
    prepare_all_args(
        ...
        args.remote_speech_tts_handler_kwargs,
    )
